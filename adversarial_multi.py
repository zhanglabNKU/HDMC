...
def training(dataset_list, cluster_pairs, nn_paras):
    ...
    Args:
        ...
        cluster_pairs: pairs of similar and noisy clusters with weights
    ...
    Returns:
        ...
        loss_transfer_list: list of contrastive loss
    ...
    netD2 = models.Discriminator(num_inputs=code_dim)
    netD3 = models.Discriminator(num_inputs=code_dim)
    if cuda:
        model.cuda()
        netD.to(device)
        netD2.to(device)
    ...
    for epoch in range(1, num_epochs + 1):
        avg_loss, avg_reco_loss, avg_tran_loss, avg_adver_loss \
            = training_epoch(epoch, model, netD, netD2, netD3, cluster_loader_dict, cluster_pairs, nn_paras)
...
def training_epoch(epoch, model, netD, netD2, netD3, cluster_loader_dict, cluster_pairs, nn_paras):
    """ Training an epoch
        Args:
            epoch: number of the current epoch
            model: autoencoder
            netD, netD2, netD3: discriminators
        ...
        Returns:
            ...
            avg_tran_loss: average contrastive loss of mini-batches
    ...
    for it in range(0, num_iter):
        ...
        for cls in data_dict:
            ...
            discriminate_dict1[cls] = netD(reverse_code)
            discriminate_dict2[cls] = netD2(reverse_code)
            discriminate_dict3[cls] = netD3(reverse_code)
            if cls <= batch_split[0]:
                real_list1.append(discriminate_dict1[cls].cpu().detach().numpy())
            else:
                fake_list1.append(discriminate_dict1[cls].cpu().detach().numpy())
            if batch_split[0] < cls <= batch_split[1]:
                real_list2.append(discriminate_dict2[cls].cpu().detach().numpy())
            else:
                fake_list2.append(discriminate_dict2[cls].cpu().detach().numpy())
            if cls > batch_split[1]:
                real_list3.append(discriminate_dict3[cls].cpu().detach().numpy())
            else:
                fake_list3.append(discriminate_dict3[cls].cpu().detach().numpy())
        ...
        real_set1 = torch.from_numpy(np.array(real_list1)).view(-1, 1).cuda()
        fake_set1 = torch.from_numpy(np.array(fake_list1)).view(-1, 1).cuda()
        real_set2 = torch.from_numpy(np.array(real_list2)).view(-1, 1).cuda()
        fake_set2 = torch.from_numpy(np.array(fake_list2)).view(-1, 1).cuda()
        real_set3 = torch.from_numpy(np.array(real_list3)).view(-1, 1).cuda()
        fake_set3 = torch.from_numpy(np.array(fake_list3)).view(-1, 1).cuda()

        label1 = torch.full((real_set1.shape[0],), real_label, dtype=real_set1.dtype, device=device)
        label2 = torch.full((real_set2.shape[0],), real_label, dtype=real_set2.dtype, device=device)
        label3 = torch.full((real_set3.shape[0],), real_label, dtype=real_set3.dtype, device=device)
        errD_real = bce(real_set1, label1) + bce(real_set2, label2) + bce(real_set3, label3)

        label1 = torch.full((fake_set1.shape[0],), fake_label, dtype=fake_set1.dtype, device=device)
        label2 = torch.full((fake_set2.shape[0],), fake_label, dtype=fake_set2.dtype, device=device)
        label3 = torch.full((fake_set3.shape[0],), fake_label, dtype=fake_set3.dtype, device=device)
        errD_fake = bce(fake_set1, label1) + bce(fake_set2, label2) + bce(fake_set3, label3)
