...
def evaluate_scores(div_ent_code, sil_code, cell_labels, dataset_labels, num_datasets, div_ent_dim, sil_dim, sil_dist,
    ...
    Args:
        ...
        normalize: whether to nomalize the data before k-means clustering
    Returns:
    ...
    # clustering and calculate ARI and NMI
    kmeans = KMeans(n_clusters=np.unique(cell_labels).shape[0], random_state=0)
    if normalize:
        normalize = Normalizer()
        sil_code = normalize.fit_transform(sil_code)
    cluster_ids = kmeans.fit_predict(sil_code)
    ari_score = adjusted_rand_score(cell_labels, cluster_ids)
    ...
    return div_score, ent_score, sil_score, ari_score
