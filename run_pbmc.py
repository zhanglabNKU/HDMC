...
# IMPORTANT PARAMETER
high_thr = 0.9  # higher threshold in the paper
low_thr = 0.87  # lower threshold in the paper
...
batch_split = 12  # number of clusters in the first batch
log_interval = 1
# CUDA
...
if __name__ == '__main__':
    data_folder = 'pbmc/'
    dataset_file_list = ['pbmc_ctrl_seurat.csv', 'pbmc_stim_seurat.csv']
    ...
    # plot results
    cell_type_dict = {1: 'B cells', 2: 'CD14+', 3: 'CD4 T', 4: 'CD8 T', 5: 'Dendritic', 6: 'FCGR3A+',
                      7: 'Megakaryocytes', 8: 'NA', 9: 'NK'}
    dataset_dict = {1: 'ctrl', 2: 'stim'}
    ...
    div_score, ent_score, sil_score, ari_score \
        = evaluate_scores(umap_code, code, cell_labels, dataset_labels, num_datasets, 20, 20, 'cosine', normalize=True)
    ...
    print('uncorrected')
    div_score, ent_score, sil_score, ari_score \
        = evaluate_scores(umap_uncorrected, data, cell_labels, dataset_labels, num_datasets, 20, 20, 'cosine', True)
    print('divergence_score: {:.3f}, entropy_score: {:.3f}, silhouette_score: {:.3f}, ari_score: {:.3f}, '
          .format(div_score, ent_score, sil_score, ari_score))
