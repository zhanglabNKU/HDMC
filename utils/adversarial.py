...
def training(dataset_list, cluster_pairs, nn_paras):
    ...
    loss_transfer_list = []
    loss_adver_list = []
    cluster_number = len(np.unique(dataset_list[0]['cluster_labels']))
    for epoch in range(1, num_epochs + 1):
        avg_loss, avg_reco_loss, avg_tran_loss, avg_adver_loss \
            = training_epoch(epoch, model, netD, cluster_loader_dict, cluster_pairs, nn_paras, cluster_number)
...
def training_epoch(epoch, model, netD, cluster_loader_dict, cluster_pairs, nn_paras, cluster_number):
    """ Training an epoch
        Args:
            ...
            cluster_pairs: pairs of similar clusters with weights
            nn_paras: parameters for neural network training
            cluster_number: number of clusters in the first batch
        Returns:
