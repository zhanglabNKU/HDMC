...
def multi_training(dataset_list, cluster_pairs, nn_paras):
    ...
    netD_list = []
    for i in range(len(dataset_list)):
        netD_list.append(models.Discriminator(num_inputs=code_dim))
    if cuda:
        model.cuda()
        for i in range(len(dataset_list)):
            netD_list[i].to(device)
    ...
    for epoch in range(1, num_epochs + 1):
        avg_loss, avg_reco_loss, avg_tran_loss, avg_adver_loss \
            = training_epoch(epoch, model, netD_list, cluster_loader_dict, cluster_pairs, nn_paras, len(dataset_list),
                             max_cluster_numbers)
...
def training_epoch(epoch, model, netD_list, cluster_loader_dict, cluster_pairs, nn_paras, data_number,
                   max_cluster_numbers):
    """ Training an epoch
        Args:
            ...
            cluster_pairs: pairs of similar clusters with weights
            nn_paras: parameters for neural network training
            data_number: number of input datasets
            max_cluster_numbers: the max cluster id in each dataset
        Returns:
    ...
    for i in range(len(netD_list)):
        optimizer.add_param_group({'params': netD_list[i].parameters()})
    bce = nn.BCELoss()
    ...
    for it in range(0, num_iter):
        ...
        code_dict = {}
        reconstruct_dict = {}
        discriminate_dict_list = []
        real_list = []
        fake_list = []
        for i in range(data_number):
            discriminate_dict_list.append({})
            real_list.append([])
            fake_list.append([])
        label_list = []
        code_list = []
        ...
        for cls in data_dict:
            ...
            for i in range(data_number):
                discriminate_dict_list[i][cls] = netD_list[i](reverse_code)
            for i in range(1, data_number+1):
                if max_cluster_numbers[i-1] < cls <= max_cluster_numbers[i]:
                    real_list[i-1].append(discriminate_dict_list[i-1][cls].cpu().detach().numpy())
                else:
                    fake_list[i-1].append(discriminate_dict_list[i-1][cls].cpu().detach().numpy())
        ...
        for i in range(len(real_list)):
            real_set.append(torch.from_numpy(np.array(real_list[i])).view(-1, 1).squeeze().cuda())
            fake_set.append(torch.from_numpy(np.array(fake_list[i])).view(-1, 1).squeeze().cuda())
            real_labels.append(torch.full((real_set[i].shape[0],), 1, dtype=real_set[i].dtype, device=device))
            fake_labels.append(torch.full((fake_set[i].shape[0],), 0, dtype=fake_set[i].dtype, device=device))

        errD_real = 0
        for i in range(len(real_labels)):
            errD_real += bce(real_set[i], real_labels[i])
        errD_fake = 0
        for i in range(len(fake_labels)):
            errD_fake += bce(fake_set[i], fake_labels[i])
